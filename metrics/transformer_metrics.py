...
def _pad_tensors_to_same_length(x, y):
    ...
    :return:
    """
    with tf.name_scope('pad_tensors_to_same_length'):
...
def padded_cross_entropy_loss(logits, labels,smoothing, vocab_size):
    ...
    with tf.name_scope('loss'):
        logits, labels = _pad_tensors_to_same_length(
            logits,
            labels
        )
...
def padded_accuracy_top_k(logits, labels, k):
    with tf.name_scope('padded_accuracy_top_k'):
        logits, labels = _pad_tensors_to_same_length(logits, labels)
        weights = tf.cast(tf.not_equal(labels, 0), tf.float32)
        ...
        same_top_k = tf.reduce_sum(same, axis=-1)
        return same_top_k, weights


def padded_accuracy_top_5(logits, labels):
    return padded_accuracy_top_k(logits, labels, 5)
...
def padded_sequence_accuracy(logits, labels):
    with tf.name_scope('padded_sequence_accuracy'):
        ...
        padded_labels = tf.cast(labels, tf.int32)
        not_correct = tf.cast(
            tf.not_equal(outputs, padded_labels),
            tf.float32
        ) * weights
...
class MetricLayer(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.metric_mean_fns = [
            ...
            (tf.keras.metrics.Mean('accuracy_top_5'), padded_accuracy_top_5),
